        ...
        <tr>
           ...
           <td width=700 colspan=3>
           		<c:if test="${vo.image==null}">
           			<img src="http://placehold.it/200x150" width=200 height=150 id="image" style="float:left; margin-left:24px;">
           		</c:if>
           		<c:if test="${vo.image!=null}">
           			<img src="/product/img/${vo.image}" width=200 height=150 id="image" style="float:left; margin-left:24px;"/>
           		</c:if>
		...
		{{#each array}}
			<div class="replyBox" style="font-size:15px;" writer={{writer}} rn={{rn}} editChk={{editchk}}>
				 	<div class="replyMenu" score={{score}} style="font-size:15px;text-align:left;overflow:hidden;padding:10px;width:700px;margin:0px auto;border:1px solid lightgrey; box-shadow:0px 0px 10px lightgrey;">
						...
						<button class="btnDelete" rn={{rn}} rid={{rid}} writer={{writer}} style="{{hiddener writer}}float:right;padding:5px;">댓글 삭제 </button>
						<button class="btnEdit" rn={{rn}} rid={{rid}} score={{score}} writer={{writer}} content={{content}} style="{{hidden writer}}float:right;padding:5px; border-radius:20px;">댓글 수정</button>
